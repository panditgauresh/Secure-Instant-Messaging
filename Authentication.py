...
class Authentication():
    ...
    def __init__(self, addr, cryto_service):
        ...
        if self.stage == 0:
            ...
            if self.ra.challengeComm.isChallengeMatched(k, ind, c_ans) or True:  # TODO for testing
                ...
                if addr == "{}:{}".format(self.addr[0], self.addr[1]) and False: # TODO for testing
                    dh_pri_key = self.cryto_service.get_dh_pri_key()
                    dh_pub_server = self.cryto_service.get_dh_pub_key(dh_pri_key)
                    secret = self.cryto_service.get_dh_secret(dh_pri_key, dec_dh_pub_client)
